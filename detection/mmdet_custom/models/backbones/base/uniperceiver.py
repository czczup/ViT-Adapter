...
class BertSelfWindowAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = window_partition(
            x,
            window_size=self.window_size)  # nW*B, window_size, window_size, C
        x = x.view(-1, N_, C)

        q = self.query(x).view(-1, N_, self.num_heads,
                               C // self.num_heads).permute(0, 2, 1, 3)
        k = self.key(x).view(-1, N_, self.num_heads,
                             C // self.num_heads).permute(0, 2, 1, 3)
        v = self.value(x).view(-1, N_, self.num_heads,
                               C // self.num_heads).permute(0, 2, 1, 3)
        ...
        x = (attn @ v).transpose(1, 2).reshape(-1, self.window_size,
                                               self.window_size, C)
...
class PatchEmbed(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embed = self._get_pos_embed(self.pos_embed.weight.unsqueeze(0),
                                        H // 16, W // 16)
        x = x + pos_embed
        return x, H // 16, W // 16
...
class UnifiedBertEncoder(nn.Module):
    def __init__(self,
                 ...
                 embed_layer=VisualPatchEmbedding,
                 window_attn=False,
                 window_size=14,
                 pretrained=None):
        ...
        for i in range(depth):
            layers.append(
                BertLayer(hidden_size=embed_dim,
                          intermediate_size=int(embed_dim * mlp_ratio),
                          num_attention_heads=num_heads,
                          drop_path_ratio=drop_path_rate,
                          windowed=window_attn[i],
                          window_size=window_size[i]))

        self.layers = nn.ModuleList(layers)
        self.visual_embed = embed_layer(in_dim=in_chans,
                                        out_dim=embed_dim,
