...
class HybridTaskCascadeAug(CascadeRCNN):
    ...
    def aug_segm_forward(self, img_feats, det_bboxes, det_labels,
    ...
    def aug_test_vote(self, imgs, img_metas, rescale=False):
        ...
        for i, (x, img_meta) in enumerate(zip(feats, img_metas)):
            ...
            det_bboxes, det_scores, semantic_feat = self.aug_bbox_forward(
                x, proposal_list, img_meta, rescale=False)
            restored_bboxes, _ = self.merge_aug_results([det_bboxes],
                                                        [det_scores],
                                                        [img_meta])
            keeped = self.remove_boxes(restored_bboxes,
                                       self.test_cfg.aug.scale_ranges[i // 2])
            det_bboxes, det_scores = det_bboxes[keeped, :], det_scores[
                keeped, :]
        ...
        if self.with_mask:
            segm_results = self.aug_segm_forward(feats, _det_bboxes,
                                                 det_labels, semantic_feats,
                                                 img_metas)
