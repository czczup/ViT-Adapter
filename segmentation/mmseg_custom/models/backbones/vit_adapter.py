...
class InteractionBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.extractor = Extractor(dim=dim, n_levels=1, num_heads=num_heads,
        ...
        if extra_extractor:
            self.extra_extractors = nn.Sequential(*[
                Extractor(dim=dim, num_heads=num_heads, n_points=n_points,
...
class ViTAdapter(TIMMVisionTransformer):
    ...
    def _deform_inputs(self, x):
        ...
        reference_points = self._get_reference_points([(h // 16, w // 16)], x.device)
